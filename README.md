...
把 **Google Antigravity 模型接入 DeepSeek Harness（DSH）** —— 支持**多账号智能池化轮换**、**官方双 Bucket 配额监控（5小时滚动 + 7天周额度）**、**浏览器免粘贴一键登录**，由官方未修改的 `agy` CLI 驱动，完整支持流式输出、思考折叠（thinking）、原生工具卡片与精确 token 用量。
...
| 前置项 | 说明 / 验证方式 |
| --- | --- |
| **1. DeepSeek Harness (DSH)** | 你正在使用的 DSH 客户端；`dsh --version` 可验证 |
| **2. Node.js ≥ 24** | DSH 依赖环境；`node --version` 验证 |
| **3. Google Antigravity `agy` CLI** | 按 [Google 官方安装文档](https://antigravity.google/docs/cli/install) 安装（支持 macOS / Linux / Windows）。`agy --version` 能输出版本号即表示安装成功 |
...
| Requirement | Details |
| --- | --- |
| **1. DeepSeek Harness (DSH)** | Your current DSH environment (`dsh --version`) |
| **2. Node.js ≥ 24** | Required runtime (`node --version`) |
| **3. Google Antigravity `agy` CLI** | Installed per [Google's official guide](https://antigravity.google/docs/cli/install). Verified with `agy --version` |
